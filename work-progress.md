...
```c++
/*
1.05 - 1.06(周二、周三) : 实现ffmpeg-GPU解码，24个线程调用GPU解码依然耗时严重100ms，对耗时未起作用

1.07 - 1.07(周四) : 定位到耗时比较严重的函数为av_buffersink_get_frame()，暂时没有找到解决该函数耗时的方案(可以考虑调用GPU版的filter，暂时没有跑通)，通过SDL显示24路视频暂时解决了这个问题

1.08 - 1.08(周五) : 安装和调试Apache服务器
*/
```
...
1.15 - 1.15(周五) : 写毕业论文大纲
*/
```

```cpp
/*
1.17 - 1.18(周日，周一) : 优化视频拼接算法ffmpeg-filter -> memcpy 24个Tile(640x480)拼接一帧，耗时从[90-110]ms -> [2-6]ms；修改了一下编码器参数提高编码器性能1920x1080YUV420px1567frame -> h264[min_dur: 0ms; max_dur: 196ms; ave_dur: 17ms] -> [min_dur: 0ms; max_dur: 29ms; ave_dur: 4ms]；1920x1080YUV420px1567frame -> h265[min_dur: min_dur: 0ms; max_dur: 331ms; ave_dur: 79ms] -> [min_dur: 0ms; max_dur: 94ms; ave_dur: 23ms]；研究了一下ffmpeg-GPU编码的代码，上面的性能满足要求了，就没有去测试，后面有时间测试一下
...
1.23 - 1.24(周六、周日) : 写论文(绪论)

1.25 - 1.25(周一) : 将最新版的libdash-networkpart-test加入到播放器平台

1.26 - 1.27(周二、周三) : 看师姐的码率分配算法论文和代码

1.28 - 1.28(周四) : 搭建训练深度增强学习算法的TensorFlow平台，学习OpenGL

1.29 - 1.29(周五) : 学习OpenGL，研究OpenVR的VR-HMD播放例程

1.30 - 1.30(周六) : 写VR-HMD播放代码
*/
```

```c++
/*
1.31 - 2.01(周日、周一) : 完成VR-HMD播放代码
